...
//    dis: (b_s, g_n, g_k)
//    idx: (b_s, g_n, g_k)
__global__ void bpa_ball_query_kernel(int b_s, int c_s, int p_n, int g_n, int g_k, float g_r, const PointType * __restrict__ pnt, const PointType * __restrict__ qry, PointType * __restrict__ dis, IndexType * __restrict__ idx) {
  int b_i = blockIdx.y;
  ...
  for (int k_i = 0; k_i < g_k; ++k_i) {
    best_dis[k_i] = (PointType) MAX_INF;
    best_idx[k_i] = (IndexType) p_n;
  }
  PointType g_d = POW_2(g_r);
  int val_hp_sz = 0;
...
void bpa_ball_query_kernel_wrapper(int b_s, int c_s, int p_n, int g_n, int g_k, float g_r, const PointType * pnt, const PointType * qry, PointType * dis, IndexType * idx) {
  ...
  bpa_ball_query_kernel<<<blocks, threads>>>(b_s, c_s, p_n, g_n, g_k, g_r, pnt, qry, dis, idx);
  CHECK_ERROR();
}