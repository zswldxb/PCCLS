...
class BasicLayer(nn.Module):
    def __init__(self, d_in: int, d_out: int, norm: str='BN', act: str='ReLU', *, ndim: int=1, channels_first: bool=False):
        super().__init__()
        part_proj_fn = partial(proj_fn, bias=False, ndim=ndim, channels_first=channels_first)
        part_bn_fn = partial(norm_fn, NAME=norm, ndim=ndim, channels_first=channels_first)
...
class PointNetV2Encoder(nn.Module):
    def __init__(self, d_in: int, d_feats: List[int], norm: str='BN', act: str='ReLU', *, ndim: int=1, channels_first: bool=False):
        ...
        self.stgs = nn.ModuleList([BasicLayer(d_in if i == 0 else d_feats[i-1], d_feats[i], norm=norm, act=act, ndim=ndim, channels_first=channels_first) for i in range(self.n_stgs)])
...
class PointNetV2(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.feat = PointNetV2Encoder(cfg.d_in, cfg.d_feats)
